...
type OrderRepository interface {
	CreateOrder(ctx context.Context, order ordermodule.Order, orderItems []ordermodule.OrderItem) (ordermodule.Order, error)
	GetOrderByID(ctx context.Context, id uuid.UUID) (ordermodule.Order, error)
	GetOrdersByUserID(ctx context.Context, userID uuid.UUID) ([]ordermodule.Order, error)
	GetAllOrders(ctx context.Context) ([]ordermodule.Order, error)
	UpdateOrder(ctx context.Context, order ordermodule.Order) (ordermodule.Order, error)
	UpdateOrderStatus(ctx context.Context, id uuid.UUID, status string) (ordermodule.Order, error)
}
...
func (r *orderRepository) CreateOrder(ctx context.Context, order ordermodule.Order, orderItems []ordermodule.OrderItem) (ordermodule.Order, error) {
	tx, err := r.rawDB.BeginTx(ctx, nil)
	if err != nil {
		return ordermodule.Order{}, fmt.Errorf("failed to begin transaction: %w", err)
	}
	defer func() {
		if p := recover(); p != nil {
	...
	if err != nil {
		return ordermodule.Order{}, fmt.Errorf("failed to create order: create order error")
	}
	...
	for _, item := range orderItems {
		...
		// if err != nil {
		// 	return ordermodule.Order{}, fmt.Errorf("failed to generate UUID for order item: %w", err)
		// }
		...
		if err != nil {
			return ordermodule.Order{}, fmt.Errorf("failed to create order item: create order item error")
		}
	}
...
func (r *orderRepository) GetOrderByID(ctx context.Context, id uuid.UUID) (ordermodule.Order, error) {
	order, err := r.q.GetOrderByID(ctx, r.rawDB, id) // Pass r.rawDB as DBTX
	if err != nil {
		return ordermodule.Order{}, fmt.Errorf("failed to get order by ID: %w", err)
	}

	orderItems, err := r.q.GetOrderItemsByOrderID(ctx, r.rawDB, order.ID) // Pass r.rawDB as DBTX
	if err != nil {
		return ordermodule.Order{}, fmt.Errorf("failed to get order items for order ID %s: %w", order.ID, err)
	}
...
func (r *orderRepository) GetOrdersByUserID(ctx context.Context, userID uuid.UUID) ([]ordermodule.Order, error) {
	...
	var domainOrders []ordermodule.Order
...
func (r *orderRepository) GetAllOrders(ctx context.Context) ([]ordermodule.Order, error) {
...
func (r *orderRepository) UpdateOrder(ctx context.Context, order ordermodule.Order) (ordermodule.Order, error) {
	...
	if err != nil {
		return ordermodule.Order{}, fmt.Errorf("failed to update order: %w", err)
	}
	return mapSQLCToDomainOrder(updatedOrder), nil
}

// UpdateOrderStatus updates the status of an order.
func (r *orderRepository) UpdateOrderStatus(ctx context.Context, id uuid.UUID, status string) (ordermodule.Order, error) {
	updatedOrder, err := r.q.UpdateOrderStatus(ctx, r.rawDB, sqlc.UpdateOrderStatusParams{ // Pass r.rawDB as DBTX
		ID:     id,
		Status: ordermodule.OrderStatus(status),
	})
	if err != nil {
		return ordermodule.Order{}, fmt.Errorf("failed to update order status: %w", err)
	}
	return mapSQLCToDomainOrder(updatedOrder), nil
}

func mapSQLCToDomainOrder(order sqlc.Order) ordermodule.Order {
	total, _ := strconv.ParseFloat(order.Total, 64) // Convert string to float64
	return ordermodule.Order{
		ID:        order.ID.String(),
		UserID:    order.UserID.String(),
		Total:     total,
		Status:    ordermodule.OrderStatus(order.Status),
		CreatedAt: order.CreatedAt.Time,
	}
}

func mapSQLCToDomainOrderItems(sqlcOrderItems []sqlc.OrderItem) []ordermodule.OrderItem {
	var domainOrderItems []ordermodule.OrderItem
	for _, item := range sqlcOrderItems {
		price, _ := strconv.ParseFloat(item.Price, 64) // Convert string price to float64
		domainOrderItems = append(domainOrderItems, ordermodule.OrderItem{
			ID:        item.ID.String(),
