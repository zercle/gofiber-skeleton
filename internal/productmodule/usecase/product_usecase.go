...
// productUseCase implements the productmodule.ProductUseCase interface.
type productUseCase struct {
	productRepo productmodule.ProductRepository
}

// NewProductUseCase creates a new ProductUseCase instance.
func NewProductUseCase(productRepo productmodule.ProductRepository) productmodule.ProductUseCase {
...
func (uc *productUseCase) CreateProduct(name, description string, price float64, stock int, imageURL string) (*productmodule.Product, error) {
	// Validate input
	if name == "" {
...
func (uc *productUseCase) GetProduct(id string) (*productmodule.Product, error) {
	if id == "" {
...
func (uc *productUseCase) GetAllProducts() ([]*productmodule.Product, error) {
...
func (uc *productUseCase) UpdateProduct(id, name, description string, price float64, stock int, imageURL string) (*productmodule.Product, error) {
	// Validate input
	if id == "" {
		return nil, errors.New("product ID is required")
	}
