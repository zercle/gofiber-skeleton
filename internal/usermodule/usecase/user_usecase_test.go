...
func TestUserUseCase_Register(t *testing.T) {
	...
	t.Run("successful registration", func(t *testing.T) {
		...
		mockUserRepo.EXPECT().Create(gomock.Any()).DoAndReturn(func(user *usermodule.User) (*usermodule.User, error) {
	...
	t.Run("registration with empty role defaults to customer", func(t *testing.T) {
		mockUserRepo.EXPECT().GetByUsername(username).Return(nil, errors.New("not found")) // User does not exist
		mockUserRepo.EXPECT().Create(gomock.Any()).DoAndReturn(func(user *usermodule.User) (*usermodule.User, error) {
			assert.Equal(t, usermodule.RoleCustomer, user.Role)
			return user, nil
		})
	...
	t.Run("username already exists", func(t *testing.T) {
		existingUser := &usermodule.User{Username: username}
...
func TestUserUseCase_GetByID(t *testing.T) {
	...
	expectedUser := &usermodule.User{ID: userID, Username: "testuser"}
...
func TestUserUseCase_UpdateRole(t *testing.T) {
	...
	t.Run("successful role update", func(t *testing.T) {
		...
		mockUserRepo.EXPECT().Update(gomock.Any()).DoAndReturn(func(user *usermodule.User) error {
			assert.Equal(t, usermodule.RoleAdmin, user.Role)
			assert.False(t, user.UpdatedAt.IsZero())
			return nil
		})

		err := usecase.UpdateRole(userID, usermodule.RoleAdmin)
		require.NoError(t, err)
	})
